...
public class BranchRenderer extends AbstractRenderer {
	...
	public void paint(GC gc, Object value) {
		...
		//gc.drawRectangle(new Rectangle(bounds.x, bounds.y + 1, bounds.width, bounds.height - 3));
		//gc.drawRectangle(bounds);
		//gc.setForeground(Display.getCurrent().getSystemColor(SWT.COLOR_DARK_RED));
		//gc.drawRectangle(toggleBounds);
...
//						: SWT.COLOR_WIDGET_NORMAL_SHADOW));
		...
		//int dy = bounds.y % 2;
		//int dy = bounds.y;
		//String s = (new Integer(dy)).toString() + " - " + value + " - " + bounds + " - " + toggleBounds; 
		//String s = indent + " - " + value + " - " + bounds + " - " + toggleBounds;
...
//			yToggleBottom += dy == 1 ? -1 : 1;
		...
		for (int i = 0; i < branches.length; i++) {
			...
			int xToggleRight = xLeft + toggleBounds.width - 2;
...
//			}
			...
			if ((branches[i] & H_FULL) == H_FULL) {
				...
				//System.out.println(xLeft + ", " + yMiddle + ", " + xRight + ", " + yMiddle);
			...
			if ((branches[i] & H_RIGHT) == H_RIGHT) {
				...
				gc.drawLine(xMiddleBranch, yMiddle, xRight + 4, yMiddle);
			}
			...
			if ((branches[i] & H_LEFT_TOGGLE) == H_LEFT_TOGGLE) {
				...
				gc.drawLine(xLeft + 2, yMiddle, xToggleRight, yMiddle);
			}
			//gc.drawLine(xLeft, yMiddle, xLeft, yMiddle);
			...
			if ((branches[i] & ASCENDER) == ASCENDER) {
				//gc.setForeground(Display.getCurrent().getSystemColor(SWT.COLOR_BLUE));
				gc.drawLine(xMiddle, yTop, xMiddle, yToggleTop + 1);
			}
			if ((branches[i] & DESCENDER) == DESCENDER) {
				//gc.setForeground(Display.getCurrent().getSystemColor(SWT.COLOR_DARK_GREEN));
				//System.out.println(xMiddle + ", " + yToggleBottom + ", " + xMiddle + ", " + yBottom);
