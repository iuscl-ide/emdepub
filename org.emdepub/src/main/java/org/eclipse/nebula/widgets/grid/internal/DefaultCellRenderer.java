...
//			gc.fillRectangle(getBounds().x, getBounds().y, getBounds().width, getBounds().height);
		...
		if (isTree()) {
			...
			toggleRenderer.setSelected(drawAsSelected);
		...
		if (isCheck()) {
			...
			if (isCenteredCheckBoxOnly(item)) {
				...
				checkRenderer.setBounds(getBounds().x + ((getBounds().width - checkRenderer.getBounds().width) / 2),
						(getBounds().height - checkRenderer.getBounds().height) / 2 + getBounds().y, checkRenderer.getBounds().width,
						checkRenderer.getBounds().height);
			}
			else {
				checkRenderer.setBounds(getBounds().x + x, (getBounds().height - checkRenderer.getBounds().height) / 2 + getBounds().y,
		...
		if (!isWordWrap()) {
			if ((isTree()) || (getColumn() < item.getParent().getColumnCount())) {
				...
				gc.drawString(text, getBounds().x + x + 2, getBounds().y + textTopMargin + topMargin + 2, true);
			}
		}
		else {
			/* RANGE */
...
//			}
			...
			if (drawBackground) {
				...
				gc.fillRectangle(getBounds().x + x, getBounds().y, getBounds().width, getBounds().height);
				
				gc.setBackground(backColor);
			}

			textLayout.draw(gc, getBounds().x + x + 2, getBounds().y + textTopMargin + topMargin + 2);
		}
...
// with this code:
		...
		if (!isWordWrap()) {
			...
			/* RANGE */
			plainTextWidth = plainTextWidth - 3;
	...
	private boolean overToggle(GridItem item, Point point) {
		...
		if (point.x >= x && point.x < (x + toggleRenderer.getSize().x + 2)) {
			// return true;
			int yStart = ((getBounds().height - toggleRenderer.getBounds().height) / 2);
			if (point.y >= yStart && point.y < yStart + toggleRenderer.getSize().y + 2) {
				return true;
			}
		}
